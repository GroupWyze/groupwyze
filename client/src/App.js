  ...
  render() {
    ...
    return (
      ...
      < div>
        ...
        < Route exact path="/user/:userId" userProfile={ profile } render={ (props) => (!isAuthenticated() ? ( < Redirect to="/user" />) : ( < UserPage profile={ this.props.userProfile } { ...props } />)) } />
        < Route exact path="/user" userProfile={ this.props.profile } render={ (props) => (!isAuthenticated() ? ( < Redirect to="/home" />) : ( < UserPage profile={ this.props.userProfile } { ...props } />)) } />
        < Route exact path="/shindig/:shindigId" userProfile={ this.props.profile } render={ (props) => (!isAuthenticated() ? ( < Redirect to="/user" />) : ( < Shindig profile={ this.props.userProfile } { ...props } />)) } />
        < Route exact path="/shindig" userProfile={ this.props.profile } render={ (props) => (!isAuthenticated() ? ( < Redirect to="/user" />) : ( < Shindig profile={ this.props.userProfile } { ...props } />)) } />
        </div>
      );
  }
}
